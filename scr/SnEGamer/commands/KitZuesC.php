...
class KitZuesC extends BaseCommand
{
    /** @var Charms */
    private $plugin;
        ...
        if ($sender->hasPermission("charms.command.use")) {
            if (count($args) === 1) {
                ...
                if ($target instanceof Player) {
                    $item = Item::get(Item::END_PORTAL_FRAME);
                    $item->setCustomName(TextFormat::BOLD . TextFormat::AQUA . "Zues Kit" . TextFormat::BOLD . TextFormat::WHITE . " Charm");
                    $lore = [
                        TextFormat::GRAY . "Right-Click to redeem the " . TextFormat::AQUA . "zues kit" . TextFormat::GRAY . ",",
                        TextFormat::GRAY . "and gain all the kit's items!"
                    ];
                    $item->setLore($lore);
                    $item->getNamedTag()->setInt("zeusc", 1);
                ...
                $sender->sendMessage("Usage: /zeusc <player>");
            }
        } else {
