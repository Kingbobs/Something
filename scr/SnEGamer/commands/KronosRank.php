...
class KronosRank extends BaseCommand
{
    /** @var Charms */
    private $plugin;
        ...
        if ($sender->hasPermission("charms.command.use")) {
            if (count($args) === 1) {
                ...
                if ($target instanceof Player) {
                    $item = Item::get(Item::PRISMARINE_SHARD);
                    $item->setCustomName(TextFormat::BOLD . TextFormat::LIGHT_PURPLE . "Kronos Rank" . TextFormat::BOLD . TextFormat::WHITE . " Voucher");
                    $lore = [
                        TextFormat::GRAY . "Right-Click to redeem this rank voucher",
                        TextFormat::GRAY . "to get the " . TextFormat::AQUA . "Kronos " . TextFormat::GRAY . "rank, with all of its perks!",
                        TextFormat::GRAY . " ",
                    ...
                    $item->setLore($lore);
                    $item->getNamedTag()->setInt("Kronos", 1);
                ...
                $sender->sendMessage("Usage: /kronosrc <player>");
            }
        } else {
