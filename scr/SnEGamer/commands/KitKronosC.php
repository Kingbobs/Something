...
class KitKronosC extends BaseCommand
{
    /** @var Charms */
    private $plugin;
        ...
        if ($sender->hasPermission("charms.command.use")) {
            if (count($args) === 1) {
                ...
                if ($target instanceof Player) {
                    $item = Item::get(Item::CLOCK);
                    $item->setCustomName(TextFormat::BOLD . TextFormat::LIGHT_PURPLE . "Kronos Kit" . TextFormat::BOLD . TextFormat::WHITE . " Charm");
                    $lore = [
                    ...
                    $item->setLore($lore);
                    $item->getNamedTag()->setInt("kronosc", 1);
                ...
                $sender->sendMessage("Usage: /kronosc <player>");
            }
        } else {
